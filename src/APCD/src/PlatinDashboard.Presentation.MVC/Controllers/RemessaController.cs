...
namespace PlatinDashboard.Presentation.MVC.Controllers
{
    ...
    public class RemessaController : Controller
    {
        ...
        public void DownloadExcel()
        {
            ...
            List<RemessaWeb> collection = objRemessaNeg.BuscaPorRegional(regionalcod, true, "false", mesano).OrderBy(x => x.NomeRegional).ToList<RemessaWeb>();
        ...
        public ActionResult PrintPartialViewToPdf(string id)
        {
            ...
            try
            {
                List<RemessaWeb> remessa2 = objRemessaNeg.BuscaPorRegional(regionalcod, true, "false", mesano).OrderBy(x => x.NomeRegional).ToList<RemessaWeb>();
        ...
        public ActionResult Index(string id, string busca, int pagina = 1, int tamanhoPagina = 10)
        {
            ...
            try
            {
                ...
                if (codregional == "000")
                {
                    ...
                    var valorTotal = objRemessaNeg.BuscaPorRegional(id, true, "true", mesano).OrderBy(x => x.NomeRegional).ToList<RemessaWeb>();
        ...
        //                    }
                    ...
                    remessa.ListaRemessa = objRemessaNeg.BuscaPorRegional(id, false, "true", mesano).OrderBy(x => x.NomeRegional).ToList<RemessaWeb>();
                ...
                else
                {
                    ...
                    ViewBag.ValorTotal = "R$ 0,00";
                    decimal preco, total = 0;
                    var valorTotal = objRemessaNeg.BuscaPorRegional(codregional, true, "true", mesano).OrderBy(x => x.NomeRegional).ToList<RemessaWeb>();
                    foreach (var company in valorTotal)
                    {
                        if (company.Ativo == "true")
                        {
          ...
          //                  {
            ...
            //                }
                    ...
                    Session["ValorTotal"] = ViewBag.Total;
                    remessa.ListaRemessa = objRemessaNeg.BuscaPorRegional(codregional, false, "true", mesano).OrderBy(x => x.NomeRegional).ToList<RemessaWeb>();
  ...
  //                  GeraDadosBradesco(TipoArquivo.CNAB400);
                ...
                var valorTotal2 = objRemessaNeg.BuscaPorRegional(id, true, "false", mesano).OrderBy(x => x.NomeRegional).ToList<RemessaWeb>();
                ...
                //     tamanhoPagina = Convert.ToInt32(Session["TAMPAGINA"].ToString());
                var remessa2 = objRemessaNeg.BuscaPorCodRegional(codregional1, mesano).Where(x => x.Matricula == "WSAWAD").ToList<RemessaWeb>().ToPagedList(pagina, tamanhoPagina); // = objRemessaNeg.BuscaPorCodRegional(codregional1, mesano).OrderBy(x => x.Nome).ToList<RemessaWeb>().ToPagedList(pagina, tamanhoPagina); 
                if (busca == "" || busca == null)
                {
                    remessa2 = objRemessaNeg.BuscaPorCodRegional(codregional1, mesano).OrderBy(x => x.Nome).ToList<RemessaWeb>().ToPagedList(pagina, tamanhoPagina);
                }
                else
                {
                    remessa2 = objRemessaNeg.BuscaPorCodRegional(codregional1, mesano).Where(x => x.Matricula == busca).OrderBy(x => x.Nome).ToList<RemessaWeb>().ToPagedList(pagina, tamanhoPagina);
                }
                return View(remessa2);
            }
            catch (Exception ex)
        ...
        public ActionResult IndexOriginal(string id, string myText)
        {
           ...
           //         GeraDadosBradesco(TipoArquivo.CNAB400);
                ...
                string codregional1 = ViewBag.CodRegional;
                List<RemessaWeb> remessa2 = objRemessaNeg.BuscaPorCodRegional(codregional1, mesano).OrderBy(x => x.Nome).ToList<RemessaWeb>();
                return View(remessa2);
            }
            catch (Exception ex)
            {
      ...
      //              GeraDadosBradesco(TipoArquivo.CNAB400);
        ...
        public ActionResult GeraRemessa(string id)
        {
            ...
            if (codregional == "000")
            {
                ...
                remessa.ListaRemessa = objRemessaNeg.BuscaPorRegional(codregional, true, "true", mesano).ToList<RemessaWeb>();
                ViewBag.NomeRegional = remessa.ListaRemessa[0].NomeRegional;
                ViewBag.DataVencimento = DateTime.Now.AddDays(10);
            }
            else
            {
                ViewBag.ValorTotal = "R$ 0,00";
                decimal preco, total = 0;
                var valorTotal = objRemessaNeg.BuscaPorRegional(codregional, true, "true", mesano).ToList<RemessaWeb>();
                foreach (var company in valorTotal)
                {
                    preco = Convert.ToDecimal(company.Valor);
                    total = total + preco;
    ...
    //                GeraDadosBradesco(TipoArquivo.CNAB400);
        ...
        public FileResult Export(string codigo)
        {
            ...
            List<object> customers =
                (from customer in objRemessaNeg.BuscaPorRegional(codregional, true, "true", mesano).Where(x => x.Codigo == codigo)
                 select new[] {
                        "1",  //Est
                        "LC", //ESp
                        "",   //Ser
                        customer.LCM.Trim(),//Titulo
                        "1",  // /P
                        customer.PN.Trim(), //Cliente
                        "2",  //Porta
                        "Padrao",  //Cart
                        "Baixa", //Referencia
                        "Real", //Moeda
                        customer.Valor.Trim(), //Valor
                        "0", //Factoring
                        "0", //Multa
                        "0", //Descont
                        "0", //Juros
                        ...
                        "0", //CONFINS
                        "0", //CSLL
                        customer.DataPagto.Trim(), //Liquid/Credito
            ...
            if (System.IO.File.Exists(Server.MapPath("~/Content/Arquivo/" + nomregional + "_" + hgravado + "_" + _FileName)))
            {
                ...
                valor.WriteLine(sb.ToString().Trim());
                valor.Close();
            }
            else
            {
            ...
            return File(Encoding.ASCII.GetBytes(sb.ToString().Trim()), "text/csv", "~/Content/Grid.csv");
        }
        ...
        public ActionResult VisualizarBoleto(int Id)
        {
            ...
            var codregional = Session["CODREGIONAL"].ToString();
            var nomregional = Session["REGIONAL"].ToString();
            ViewBag.RegionaisPagas = objRemessaNeg.BuscaPorRegional(codregional, true, "true", mesano).ToList<RemessaWeb>();
            string enderecos = Session["CODREGIONAL"].ToString();
            ViewBag.Enderecos = objEnderecosNeg.BuscaPorCodigo(enderecos);
        ...
        public void GeraArquivoCNAB400Bradesco()
        {
            try
            {
                ...
                string mesano = DateTime.Now.ToString("MMyyyy");
                RemessaModel model = new RemessaModel();
                remessa.ListaRemessa = objRemessaNeg.BuscaPorRegional(id, true, "true", mesano).ToList<RemessaWeb>();
